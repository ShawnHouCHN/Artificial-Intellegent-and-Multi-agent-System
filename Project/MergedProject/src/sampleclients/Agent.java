...
//		return this.solution.size();
	...
	public LinkedList<Node> SearchSingle(Strategy strategy) throws IOException {
		...
		while (true) {
			...
			if (leafNode.isSingleGoalState()) {
				...
				boolean valid=validateGoal(currentGoal,new int[]{leafNode.agentRow,leafNode.agentCol});
				if(valid)
				{
	...
	public void FindNextBoxGoal() {
		...
		while(unfinished_goals.size()!=0){
			...
			boolean valid=validateGoal(closest_goal, this.location);
...
//			}
	...
	public boolean validateGoal(Goal closest_goal, int[] agent_location){
